...
### 平行化（Parallel Distiller）

完整設計見 `docs/superpowers/specs/2026-07-16-parallel-distiller-design.md`、
plan 見 `docs/superpowers/plans/2026-07-16-parallel-distiller.md`。核心是兩
個獨立的旋鈕：一個「全域 LLM 併發上限」的裝飾器，加上一個「兩階段管線」
把每個 transcript 的準備工作平行化，同時保住寫入端的決定性。

**全域 LLM 併發上限（`distiller/limiter.ts`）**——`Semaphore` 是一個
FIFO 等待佇列的計數信號量：`acquire()` 回傳一個 release 函式，額度用完時
呼叫方進佇列排隊，release 冪等（重複呼叫安全，不會把計數多還一次）。
`withConcurrencyLimit(llm, limit)` 把任意 `LlmClient` 包一層：`describe()`
原樣轉發，`complete()` 先 `acquire()` 才呼叫底層 `llm.complete()`，
`finally` 一定 `release()`（就算 `complete()` 丟例外也會釋放額度，讓佇列
裡下一個呼叫可以繼續）。**這一層包住的是「同時有幾個 HTTP/subprocess
呼叫打向 LLM backend」，跟下面兩階段管線的 `PipelineOptions.concurrency`
是兩個不同的量**——後者只管「同時有幾個 transcript 的 Phase A 任務在跑」
（fan-out 衛生閥），前者才是真正卡住 backend 併發量的那一層。`cli.ts` 對
`run` 和 `reflect` 兩個指令都在建立 `LlmClient` 之後、傳進
`runPipeline`/`runReflect` 之前套用這個裝飾器：
```ts
const llm = withConcurrencyLimit(deps.llm ?? clientFromEnv(env), concurrency)
```
`pipeline.ts`/`reflect.ts` 本身完全不知道這層裝飾器存在，拿到的就是一個
普通的 `LlmClient`。

**兩階段管線（`pipeline.ts: runPipeline`）**——INGEST 之後的 dispatch
list（eligibility + ledger `isProcessed` 檢查）維持**循序**建立，理由是
Phase A 的併發 fan-out 和 Phase B 的 commit 順序都必須基於同一份固定、
決定性的清單（`scanSpool` 的穩定 `time_end` 排序）：

```
Phase A（bounded, concurrent, 每個 transcript 各自一個 task）
  TRIAGE → EXTRACT×N → VALIDATE → POOL-DEDUP → JUDGE
  → 產出 PreparedTranscript { meta, kept, secrets, counters, error?, ledgerable }
  全程零 store/index/ledger 寫入
        │
        ▼  Promise.all 保留輸入順序（不是完成順序）
Phase B（strictly sequential，依 dispatch = metas 原始順序）
  per transcript： RECONCILE → COMMIT（writeQuarantineEntry / reconcileCandidate / recordProcessed）
```

Phase A 用一個 `Semaphore(concurrency)`（`PipelineOptions.concurrency`，
預設 8）包住每個 transcript task 的 acquire/release，避免一次 run 對上百
個 transcript 同時開工；`Promise.all` 保證 `prepared` 陣列回來的順序就是
`dispatch`（= metas）的順序，跟哪個 task 實際先跑完無關——這正是 Phase B
決定性依賴的性質。Phase B 完全不做任何 concurrent 動作，逐一走
`prepared`，寫入邏輯跟兩階段重構前「每個 meta 的尾段」逐位元組相同。

**決定性契約（equivalence test，spec §2 的驗收線）**——併發度 1 跟 N 必須
產生**逐位元組相同**的 `store/` 樹（`memories/` + `quarantine/` +
`INDEX.md`，注入固定 `now`）以及相等的 ledger row 集合（忽略
`processed_at`）。`distiller/pipeline.test.ts` 用一個「依 prompt/system
內容比對回覆、絕不依呼叫順序」的 content-keyed FakeLlm 把這條線釘住
（`test("EQUIVALENCE: concurrency 1 vs 4 …")`），另有專門測試釘住
「Phase B 的 commit 順序永遠跟著 metas 順序走，不管 Phase A 誰先跑完」
（故意讓某個 transcript 的 fake 呼叫延遲最久，斷言最終 commit 順序沒被打
亂）以及「Phase A 對所有 transcript 都丟錯時，store 完全沒被碰過（連
quarantine 都沒有）」，用來反向證明 Phase A 真的零寫入。既有的
`LEGACY` 測試常數（`{ triage: "heuristic", extractRuns: 1, judges: 0,
concurrency: 1 }`）沒有被這次重構動過或弱化。

**`AGENT_MEMORY_CONCURRENCY` 調整指引**——預設 `8`（整數 `[1, 32]`，壞值
在 `cli.ts` 友善報錯 exit 1，語意跟其他 `numEnv` 一致）是針對自架 vLLM
調的：vLLM 真的能吃下 8 個並行請求，這也是平行化整個改動的意義所在。
`opencode-run` 這個 dev-only backend 不一樣——它的 `complete()` 每次呼叫
都是 `Bun.spawn` 開一個全新的 `opencode run` subprocess（見 `llm.ts:
createOpencodeRunClient`），把併發調高在這個 backend 下主要只是讓多個
process 搶 CPU/記憶體，不會真的變快，所以對著 `opencode-run` 跑時建議
