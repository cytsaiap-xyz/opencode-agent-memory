...
| `AGENT_MEMORY_JUDGES` | `3` | 整數 `[0, 5]` | judge panel 大小，`0` = 完全跳過 JUDGE 階段 |
...
`AGENT_MEMORY_JUDGES=0` 直接短路：不打任何 LLM，回傳
`{ salience: c.salience, panel: 0, voted: 0, usedFallback: true }`，效果
跟「全員棄權」完全一樣，只是零延遲零花費。
...
CLI 輸出格式與舊測試斷言。**已知落差**：`distill run` 印出的一行摘要
（`cli.ts`）目前只印 `triaged ${s.triagedOut}`，`poolRaw`/`triagedLlm`/
`triagedHeuristic` 這三個新欄位雖然存在於 `RunSummary` 物件、也有測試覆
蓋，但**還沒接進那一行人類看的 CLI 輸出**——要看這些細分數字目前只能讀程
式內部或改 CLI（Plan 7 final wave 記錄的已知缺口，非本次任務範圍）。
`extractor` 標籤（寫進每筆 entry 的 `provenance.extractor`）在
`judges > 0` 時會加上 ` judges:N` 後綴。
...
2. **`judges` 後綴同時出現在 secret provenance 上，是化妝式（cosmetic）
   已知瑕疵。** `pipeline.ts` 在迴圈最外層一次性組出 `extractor` 字串
   （`judges > 0` 就加 ` judges:N` 後綴），之後不管是走 JUDGE 階段的
   一般候選、還是完全不經過 JUDGE 就直接寫 quarantine 的 secret 候選
   （`quarantineEntry(sec.item, meta, now, extractor, promptHash)`），
   都共用同一個字串。結果是 secret quarantine entry 的
   `provenance.extractor` 也會標 `judges:3`，即使這筆候選從沒被任何
   judge 評過分——純粹是 provenance 文字上的誤導，不影響任何行為或計
   數，已知但刻意不修（Plan 7 Task 3 final wave 記錄）。
